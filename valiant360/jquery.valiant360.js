...
;(function ( $, THREE, Detector, window, document, undefined ) {
    ...
    Plugin.prototype = {
        ...
        onMouseMove: function(event) {
            ...
            if(this.options.clickAndDrag) {
                if(this._mouseDown) {
                    ...
                    // Calculate velocity for momentum from actual mouse movement
                    var currentTime = Date.now();
                    if(this._lastDragTime > 0) {
                        var timeDelta = currentTime - this._lastDragTime;
                        if(timeDelta > 0 && timeDelta < 100) { // Only if reasonable time delta
                            // Calculate velocity from actual mouse position change
                            var mouseDeltaX = event.pageX - this._lastDragX;
                            var mouseDeltaY = event.pageY - this._lastDragY;
                            this._velocityX = mouseDeltaX / timeDelta * 16; // Normalize to ~60fps
                            this._velocityY = mouseDeltaY / timeDelta * 16;
                        }
                    }
                    this._lastDragTime = currentTime;
                    this._lastDragX = event.pageX;
                    this._lastDragY = event.pageY;
                    
                    this._dragStart.x = event.pageX;
                    this._dragStart.y = event.pageY;
                    // Reduce sensitivity by multiplying by a factor (0.1 = 10% sensitivity)
                    this._lon -= x * 0.1;
                    this._lat += y * 0.1;
                }
            } else {
                ...
                this._lon = ( x / $(this.element).find('canvas').width() ) * 180 - 90;
                this._lat = ( y / $(this.element).find('canvas').height() ) * -90 + 45;
            }
        },
        ...
        onMouseDown: function(event) {
            ...
            this._dragStart.x = event.pageX;
            this._dragStart.y = event.pageY;
            // Clear momentum when starting new drag
            this._velocityX = 0;
            this._velocityY = 0;
            this._lastDragTime = 0;
        ...
        animate: function() {
            ...
            if(!this._mouseDown && (Math.abs(this._velocityX) > 0.01 || Math.abs(this._velocityY) > 0.01)) {
                // Apply velocity with decay
                this._lon -= this._velocityX * 0.1;
                this._lat += this._velocityY * 0.1;
                
                // Decay velocity (friction)
                this._velocityX *= 0.95;
                this._velocityY *= 0.95;
